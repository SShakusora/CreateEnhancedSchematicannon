...
public class WindBellBEMixin implements ISpecialBlockEntityItemRequirement, IPartialSafeNBT {
    ...
    public void writeSafe(CompoundTag out) {
        ...
        CompoundTag tag = self.getUpdateTag();
    ...
    @Unique
    @Nullable
    private DyeColor dyeColorFromTag(@Nullable Tag tag) {
        if (!(tag instanceof StringTag stringTag)) {
            return null;
        }
        String name = stringTag.getAsString();
    ...
    @Unique
    @Nullable
    public ItemStack getDyeItem(@Nullable Tag tag) {
        DyeColor color = dyeColorFromTag(tag);
        if (color == null) {
            return null;
        }
